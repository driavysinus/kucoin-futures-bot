...
- 💾 Персистентность: `alerts.json`, `plans.json`, `chat_ids.json` — переживают рестарт
...
WebSocket public → price_update → OrderManager.on_price_update()
        ...
        ↓
  Виртуальные SL/TP обновляются в памяти и `plans.json` (на биржу НЕ отправляются)
        ↓
...
- **SL и TP не существуют на бирже** — это виртуальные уровни. Активные планы сопровождения сохраняются в `plans.json` и после рестарта сверяются с реальными позициями на бирже. Пока процесс не работает, виртуальный SL/TP не исполняется.
- Порезы округляются вверх (`ceil`), минимум 1 контракт.
- Алерты сохраняются в `alerts.json`, планы сопровождения — в `plans.json`, chat ID — в `chat_ids.json`; эти файлы переживают рестарт.
